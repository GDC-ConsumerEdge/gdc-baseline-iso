...
function validate_config() {
    if [[ -z "${GRUB_PARTITION}" ]]; then
        echo "GRUB_PARTITION size variable has not been set in 'settings.sh'"
        exit 1
    fi
    if [[ -z "${PRIMARY_PARTITION}" ]]; then
        echo "PRIMARY_PARTITION size variable has not been set in 'settings.sh'"
        exit 1
    fi
    if [[ -z "${AUX_PARTITION}" ]]; then
        echo "AUX_PARTITION size variable has not been set in 'settings.sh'"
        exit 1
    fi
}
...
function install_packages() {
    if [ ! -f ./.apt_updated ]; then
        sudo apt update -y && \
        sudo apt install syslinux-utils genisoimage whois p7zip-full diceware xorriso wget isolinux whois -y && \
        touch ./.apt_updated
    fi
}

function install_packages_ubuntu_container() {
    if [ ! -f /tmp/.apt_updated ]; then
        sudo apt update -y
        sudo apt install genisoimage p7zip-full whois diceware xorriso wget isolinux -y
        touch ./.apt_updated
    fi
}
...
function xorriso_make() {
    ...
    if [[ "${UBUNTU_VERSION%%.*}" == "20" ]]; then
        ...
        #static vars retreived from running this command on the ubuntu ISO
        ...
        #build the ISO now
        #echo "Building the Jammy ISO file now!!"
        xorriso -as mkisofs \
        -V "${HOST}" \
        -o ${DESTINATION_FILE} \
        --grub2-mbr MBR.img \
...
function build_iso() {
    ...
    echo "7z x $ISO_NAME -x'![BOOT]' -o$BUILD_DIR"
    ...
    #isolinux isnt in jammy anymore - only run if this is a focal ISO
    ...
    cd $BUILD_DIR/dists/*/main/binary-amd64
    gunzip -k Packages.gz
    cd -
    cd $BUILD_DIR/dists/*/restricted/binary-amd64
    gunzip -k Packages.gz
    cd -
    ...
    do
        ...
        sed -i "s|hostname: ${HOST_FINAL}|hostname: __HOSTNAME__|g" $REPO_DIR/user-data # reset the file
    done
